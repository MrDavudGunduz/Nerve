...
//
//  Note: Wrapped in #if canImport(UIKit) because ClusterAnnotationView uses
//  UIColor, which is unavailable when running `swift test` on a macOS host.
//  These tests still run in full on iOS simulators and physical devices via Xcode.
//
